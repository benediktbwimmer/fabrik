...
PY
  ...
  for attempt in 1 2 3 4 5; do
    ...
    if [[ "$attempt" -lt 5 ]]; then
      echo "[interceptor-pressure-drill] trigger attempt $attempt failed with status $status; retrying" >&2
      sleep 2
      continue
    fi
    echo "[interceptor-pressure-drill] trigger failed with status $status" >&2
    cat "$response_path" >&2
    return 1
  done
}
...
checks = {
    ...
    "query_before_signal_shows_initial_state": pre_value == {"value": "draft"},
    "workflow_preserved_artifact_after_restart": workflow_post_restart.get("artifact_hash") == meta["artifact_hash"],
    "query_after_restart_preserves_initial_state": restart_value == {"value": "draft"},
    ...
    "query_after_complete_shows_interceptor_commit": complete_value == {"value": "approved"},

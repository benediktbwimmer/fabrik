...
usage() {
  cat >&2 <<'EOF'
usage: scripts/run-alpha-mixed-build-drill.sh [options]
...
REPO_V1="crates/fabrik-cli/test-fixtures/temporal-payload-path-qualified"
REPO_V2="crates/fabrik-cli/test-fixtures/temporal-payload-path-qualified-v2"
OUTPUT_DIR="target/alpha-drills/temporal-payload-path-mixed-build"
TENANT_ID="alpha-mixed-build-$(python3 - <<'PY'
import uuid
...
BUILD_V1="alpha-payload-path-mixed-v1"
BUILD_V2="alpha-payload-path-mixed-v2"
SKIP_STACK=0
SKIP_RESTART=0
...
echo "[mixed-build-drill] started $(timestamp)"
echo "[mixed-build-drill] repo_v1=$REPO_V1 repo_v2=$REPO_V2 tenant=$TENANT_ID api=$API_URL output=$OUTPUT_DIR_ABS"
...
trigger_workflow() {
  ...
  local instance_id=$2
  local workflow_task_queue=$3
  local name_arg=$4
  local memo_json=$5
  local search_json=$6
  local out_prefix=$7
  local request_id="${out_prefix//[^a-zA-Z0-9]/-}-trigger"

  local payload
  payload="$(python3 - "$TENANT_ID" "$instance_id" "$workflow_task_queue" "$name_arg" "$memo_json" "$search_json" "$request_id" <<'PY'
import json
import sys

tenant_id, instance_id, workflow_task_queue, name_arg, memo_json, search_json, request_id = sys.argv[1:8]
print(json.dumps({
    "tenant_id": tenant_id,
    "instance_id": instance_id,
    "workflow_task_queue": workflow_task_queue,
    "input": [name_arg, False],
    "memo": json.loads(memo_json),
    "searchAttributes": json.loads(search_json),
...
output_path, repo_v1, repo_v2, tenant_id, v1_definition_id, v1_version, v1_artifact_hash, v2_definition_id, v2_version, v2_artifact_hash, workflow_task_queue, old_instance_id, old_run_id, new_instance_id, new_run_id, rollback_instance_id, rollback_run_id = sys.argv[1:]
payload = {
    "repo_v1": repo_v1,
    "repo_v2": repo_v2,
    "tenant_id": tenant_id,
...
checks = {
    ...
    "old_run_completed_with_v1_output": old_workflow_post_complete["output"] == "hello alice",
    "new_run_completed_with_v2_output": new_workflow_post_complete["output"] == "hello bob from build v2",
    ...
    "rollback_run_completed_with_v1_output": rollback_workflow["output"] == "hello carol",
...
summary = {
    ...
    "artifacts": {
        "v1": meta["v1"]["artifact_hash"],
        "v2": meta["v2"]["artifact_hash"],
    },
    "instances": {
        "old": meta["v1"]["instance_id"],
        "new": meta["v2"]["instance_id"],
        "rollback": meta["rollback"]["instance_id"],
    },
    "checks": checks,
    "evidence": {
        "qualification_verdicts": {
            "v1": v1_report["alpha_qualification"]["verdict"],
            "v2": v2_report["alpha_qualification"]["verdict"],
        },
...
markdown = "\n".join([
    "# Alpha Mixed-Build Drill Report",
    "",
    ...
    *[f"- `{name}`: `{'passed' if ok else 'failed'}`" for name, ok in checks.items()],
    ...
    f"- Rollback run artifact: `{rollback_routing['artifact_hash']}`",
])

(out_dir / "alpha-mixed-build-drill-report.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
(out_dir / "alpha-mixed-build-drill-report.md").write_text(markdown + "\n", encoding="utf-8")
...
if [[ "$SKIP_STACK" != "1" ]]; then
  if ! curl -fsS "$API_URL/health" >/dev/null 2>&1; then
    echo "[mixed-build-drill] api gateway unavailable, starting dev stack"
    scripts/dev-stack.sh up
  else
    echo "[mixed-build-drill] dev stack already reachable"
  fi
fi
...
REPORT_STATUS_V1="$(extract_field "$OUTPUT_DIR_ABS/report-fields-v1.json" status)"
if [[ "$REPORT_STATUS_V1" == "incompatible_blocked" ]]; then
  echo "[mixed-build-drill] v1 migration blocked" >&2
  exit 1
fi
...
OLD_INSTANCE_ID="alpha-mixed-old-$(python3 - <<'PY'
import uuid
print(str(uuid.uuid4())[:8])
PY
)"

echo "[mixed-build-drill] triggering old run definition=$DEFINITION_ID_V1 instance=$OLD_INSTANCE_ID queue=$WORKFLOW_TASK_QUEUE"
trigger_workflow \
  "$DEFINITION_ID_V1" \
  "$OLD_INSTANCE_ID" \
  "$WORKFLOW_TASK_QUEUE" \
...
poll_json_path "/tenants/${TENANT_ID}/workflows/${OLD_INSTANCE_ID}" \
  "data.get('run_id') == '${OLD_RUN_ID}' and data.get('status') == 'running'" \
  "$OUTPUT_DIR_ABS/old-run-workflow-pre-upgrade.json"
...
REPORT_STATUS_V2="$(extract_field "$OUTPUT_DIR_ABS/report-fields-v2.json" status)"
if [[ "$REPORT_STATUS_V2" == "incompatible_blocked" ]]; then
  echo "[mixed-build-drill] v2 migration blocked" >&2
  exit 1
fi
...
NEW_INSTANCE_ID="alpha-mixed-new-$(python3 - <<'PY'
import uuid
print(str(uuid.uuid4())[:8])
PY
)"

echo "[mixed-build-drill] triggering new run definition=$DEFINITION_ID_V2 instance=$NEW_INSTANCE_ID"
trigger_workflow \
  "$DEFINITION_ID_V2" \
  "$NEW_INSTANCE_ID" \
  "$WORKFLOW_TASK_QUEUE" \
  "bob" \
  '{"region":"us","priority":2}' \
  '{"CustomKeywordField":["candidate","v2"],"Region":"us"}' \
  "new-run"
...
poll_json_path "/tenants/${TENANT_ID}/workflows/${NEW_INSTANCE_ID}" \
  "data.get('run_id') == '${NEW_RUN_ID}' and data.get('status') == 'running'" \
  "$OUTPUT_DIR_ABS/new-run-workflow-post-upgrade.json"
...
poll_json_path "/tenants/${TENANT_ID}/workflows/${OLD_INSTANCE_ID}" \
  "data.get('run_id') == '${OLD_RUN_ID}' and data.get('status') == 'running'" \
  "$OUTPUT_DIR_ABS/old-run-workflow-post-restart.json"
poll_json_path "/tenants/${TENANT_ID}/workflows/${NEW_INSTANCE_ID}" \
  "data.get('run_id') == '${NEW_RUN_ID}' and data.get('status') == 'running'" \
  "$OUTPUT_DIR_ABS/new-run-workflow-post-restart.json"
...
echo "[mixed-build-drill] signaling old and new runs"
json_post \
  "/tenants/${TENANT_ID}/workflows/${OLD_INSTANCE_ID}/signals/approved" \
  '{"payload":true,"request_id":"signal-approved-old"}' \
  "$OUTPUT_DIR_ABS/old-run-signal-response.json"
json_post \
  "/tenants/${TENANT_ID}/workflows/${NEW_INSTANCE_ID}/signals/approved" \
  '{"payload":true,"request_id":"signal-approved-new"}' \
  "$OUTPUT_DIR_ABS/new-run-signal-response.json"

poll_json_path "/tenants/${TENANT_ID}/workflows/${OLD_INSTANCE_ID}" \
  "data.get('run_id') == '${OLD_RUN_ID}' and data.get('status') == 'completed'" \
  "$OUTPUT_DIR_ABS/old-run-workflow-post-complete.json"
poll_json_path "/tenants/${TENANT_ID}/workflows/${NEW_INSTANCE_ID}" \
  "data.get('run_id') == '${NEW_RUN_ID}' and data.get('status') == 'completed'" \
  "$OUTPUT_DIR_ABS/new-run-workflow-post-complete.json"
...
echo "[mixed-build-drill] rolling back default workflow build to $BUILD_V1"
json_post \
...
ROLLBACK_INSTANCE_ID="alpha-mixed-rollback-$(python3 - <<'PY'
import uuid
print(str(uuid.uuid4())[:8])
PY
)"

echo "[mixed-build-drill] triggering rollback run definition=$DEFINITION_ID_V1 instance=$ROLLBACK_INSTANCE_ID"
trigger_workflow \
  "$DEFINITION_ID_V1" \
  "$ROLLBACK_INSTANCE_ID" \
  "$WORKFLOW_TASK_QUEUE" \
  "carol" \
  '{"region":"eu","priority":3}' \
  '{"CustomKeywordField":["rollback","stable"],"Region":"eu"}' \
  "rollback-run"
...
poll_json_path "/tenants/${TENANT_ID}/workflows/${ROLLBACK_INSTANCE_ID}" \
  "data.get('run_id') == '${ROLLBACK_RUN_ID}' and data.get('status') == 'running'" \
  "$OUTPUT_DIR_ABS/rollback-run-workflow-post-trigger.json"
json_get "/tenants/${TENANT_ID}/workflows/${ROLLBACK_INSTANCE_ID}/routing" "$OUTPUT_DIR_ABS/rollback-run-routing-post-trigger.json"
json_post \
  "/tenants/${TENANT_ID}/workflows/${ROLLBACK_INSTANCE_ID}/signals/approved" \
  '{"payload":true,"request_id":"signal-approved-rollback"}' \
  "$OUTPUT_DIR_ABS/rollback-run-signal-response.json"
poll_json_path "/tenants/${TENANT_ID}/workflows/${ROLLBACK_INSTANCE_ID}" \
  "data.get('run_id') == '${ROLLBACK_RUN_ID}' and data.get('status') == 'completed'" \
  "$OUTPUT_DIR_ABS/rollback-run-workflow-post-complete.json"
...
echo "[mixed-build-drill] completed $(timestamp)"
echo "[mixed-build-drill] report: $OUTPUT_DIR_ABS/alpha-mixed-build-drill-report.md"

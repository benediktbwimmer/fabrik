...
REPO_PATH="crates/fabrik-cli/test-fixtures/temporal-sinks-pressure"
OUTPUT_DIR="target/alpha-drills/temporal-sinks-pressure"
TENANT_ID="sinks-pressure-$(python3 - <<'PY'
import uuid
...
trigger_workflow() {
  ...
  local payload
  payload="$(python3 - "$TENANT_ID" "$instance_id" "$queue" "$severity" "$out_prefix" <<'PY'
import json, sys
tenant_id, instance_id, queue, severity, out_prefix = sys.argv[1:6]
print(json.dumps({
    "tenant_id": tenant_id,
    ...
    "input": [severity],
    "memo": {"lane": "sinks"},
    "searchAttributes": {"PressureGroup": "sinks"},
...
PY
  ...
  for attempt in 1 2 3 4 5; do
    ...
    if [[ "$attempt" -lt 5 ]]; then
      echo "[sinks-pressure-drill] trigger attempt $attempt failed with status $status; retrying" >&2
      sleep 2
      continue
    fi
    echo "[sinks-pressure-drill] trigger failed with status $status" >&2
    cat "$response_path" >&2
    return 1
  done
}
...
checks = {
    ...
    "workflow_running_before_release": True,
    "query_before_release_shows_waiting_state": pre_value == {"severity": "critical", "released": False},
    "workflow_preserved_artifact_after_restart": workflow_post_restart.get("artifact_hash") == meta["artifact_hash"],
    "query_after_restart_preserves_waiting_state": restart_value == {"severity": "critical", "released": False},
    ...
    "workflow_completed_with_sink_output": workflow_post_complete.get("status") == "completed" and workflow_post_complete.get("output") == "alerted:critical",
    "query_after_complete_shows_released_state": complete_value == {"severity": "critical", "released": True},
...
(out_dir / "sinks-pressure-drill-report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
lines = [
    "# Sinks Pressure Drill Report",
    "",
    f"- Status: `{status}`",
...
lines.extend([
    "",
    "## Evidence",
    f"- Query values: pre-release=`{pre_value}` post-restart=`{restart_value}` post-complete=`{complete_value}`",
...
(out_dir / "sinks-pressure-drill-report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
print(json.dumps(report, indent=2))
PY
}

echo "[sinks-pressure-drill] started $(timestamp)"
echo "[sinks-pressure-drill] repo=$REPO_PATH tenant=$TENANT_ID api=$API_URL output=$OUTPUT_DIR_ABS"

if [[ "$SKIP_STACK" != "1" ]]; then
  if [[ -x ./scripts/dev-stack.sh ]]; then
    echo "[sinks-pressure-drill] restarting dev stack"
    ./scripts/dev-stack.sh down
    DEV_STACK_BUILD="${DEV_STACK_BUILD:-0}" ./scripts/dev-stack.sh up
  else
    echo "[sinks-pressure-drill] dev-stack.sh missing" >&2
    exit 1
  fi
fi
...
echo "[sinks-pressure-drill] running migration and deployment"
...
INSTANCE_ID="sinks-pressure-$(python3 - <<'PY'
import uuid
print(str(uuid.uuid4())[:8])
PY
)"

echo "[sinks-pressure-drill] triggering workflow definition=$DEFINITION_ID instance=$INSTANCE_ID queue=$WORKFLOW_TASK_QUEUE"
trigger_workflow "$DEFINITION_ID" "$INSTANCE_ID" "$WORKFLOW_TASK_QUEUE" "critical" "workflow"
...
poll_json_path "/tenants/${TENANT_ID}/workflows/${INSTANCE_ID}" "data.get('run_id') == '${RUN_ID}' and data.get('status') == 'running'" "$OUTPUT_DIR_ABS/workflow-pre-release.json"
...
echo "[sinks-pressure-drill] relaunching managed migrated workers"
...
echo "[sinks-pressure-drill] signaling release"
json_post "/tenants/${TENANT_ID}/workflows/${INSTANCE_ID}/signals/release" '{"payload":true,"request_id":"signal-release"}' "$OUTPUT_DIR_ABS/signal-response.json"
...
echo "[sinks-pressure-drill] completed $(timestamp)"
echo "[sinks-pressure-drill] report: $OUTPUT_DIR_ABS/sinks-pressure-drill-report.md"

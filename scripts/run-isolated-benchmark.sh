...
THROUGHPUT_PARTITIONS="${THROUGHPUT_OWNERSHIP_PARTITIONS:-0,1,2,3}"
...
for topic in \
  "$WORKFLOW_EVENTS_TOPIC:4" \
  "$THROUGHPUT_COMMANDS_TOPIC:4" \
  "$THROUGHPUT_REPORTS_TOPIC:4" \
  "$THROUGHPUT_CHANGELOG_TOPIC:4" \
  "$THROUGHPUT_PROJECTIONS_TOPIC:4"
do
  name="${topic%%:*}"
  partitions="${topic##*:}"

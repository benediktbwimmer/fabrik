...
EXECUTOR_PORT="${EXECUTOR_SERVICE_PORT:-3002}"
...
STREAM_ACTIVITY_WORKER_PORT="${STREAM_ACTIVITY_WORKER_SERVICE_PORT:-50054}"
...
export EXECUTOR_SERVICE_URL=http://127.0.0.1:${EXECUTOR_PORT}
export MATCHING_SERVICE_ENDPOINT=http://127.0.0.1:${MATCHING_PORT}
...
up() {
  ...
  echo "[dev-stack] starting executor-service"
  start_service \
    executor-service \
    "$LOG_DIR/executor-service.log" \
    "$EXECUTOR_PORT" \
    "${COMMON_ENV[@]}" \
    "EXECUTOR_SERVICE_PORT=$EXECUTOR_PORT" \
    "MATCHING_SERVICE_ENDPOINT=http://127.0.0.1:$MATCHING_PORT" \
    -- \
    "$BIN_DIR/executor-service"
  ...
  echo "[dev-stack] starting activity-worker-service (matching)"
  start_service \
    activity-worker-service \
